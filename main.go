...
type Training struct {
	...
	Duration time.Duration 		// продолжительность тренировки (в чем? в ч?)
...
type InfoMessage struct {
	...
	Speed float64     // средняя скорость, с которой двигался пользователь (в чем? в м/с?)
...
func (w Walking) Calories() float64 {
	return ((CaloriesWeightMultiplier * w.Weight + (w.meanSpeed() * w.meanSpeed() * KmHInMsec * KmHInMsec / w.Height) * CaloriesSpeedHeightMultiplier * w.Weight) * float64(w.Duration.Hours()) * MinInHours)
}
